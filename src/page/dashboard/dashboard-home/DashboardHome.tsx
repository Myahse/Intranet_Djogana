...
import { History, Trash2, UserPlus, Building2, Pencil, Check, X, Circle, User, KeyRound, XCircle, RotateCcw, UserMinus, Download, FileSpreadsheet, FileText } from 'lucide-react'
...
import { UsersTable } from '@/page/dashboard/dashboard-home/components/UsersTable'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const DashboardHome = (): ReactNode => {
  ...
  const handleSuspendUser = async (targetUser: { id: string; identifiant: string; is_suspended?: boolean }) => {
    ...
    const ok = await confirm({
      title: willSuspend ? `Suspendre l'utilisateur "${targetUser.identifiant}" ?` : `Réactiver l'utilisateur "${targetUser.identifiant}" ?`,
      description: willSuspend ? 'Il sera déconnecté et ne pourra plus se connecter.' : '',
      confirmLabel: willSuspend ? 'Suspendre' : 'Réactiver',
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <UsersTable
              ...
              currentIdentifiant={user?.identifiant ?? null}
              canCreateUser={canCreateUser}
              canDeleteUser={canDeleteUser}
              editingUserId={editingUserId}
              editingUserRole={editingUserRole}
              setEditingUserRole={setEditingUserRole}
              editingUserDirectionId={editingUserDirectionId}
              setEditingUserDirectionId={setEditingUserDirectionId}
              getGrantedDirectionsForUser={getGrantedDirectionsForUser}
              onManageAccess={(id) => setManagingAccessUserId(id)}
              onStartEditUser={handleStartEditUser}
              onSaveUser={handleSaveUser}
              onCancelEditUser={handleCancelEditUser}
              onToggleChief={handleToggleChief}
